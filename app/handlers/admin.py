...
async def cmd_settings(message: Message, app_ctx: AppContext) -> None:
    ...
    lines = [
        ...
        f"• VIP channel:      {s.vip_channel_id or '(unset)'}",
        f"• Public channel:   {s.free_channel_id or '(unset)'}",
        f"• Free CTA handle:  @{s.free_cta_username}"
        if s.free_cta_username
        else "• Free CTA handle:  (unset)",
...
async def cmd_active(message: Message, app_ctx: AppContext) -> None:
    ...
    for r in rows:
        lines.append(
            f"#{r.id}  {r.symbol} {r.direction}  •  {r.channel_type}\n"
...
async def cmd_close(message: Message, app_ctx: AppContext) -> None:
    ...
    if rec.status != "active":
        await message_answer_logged(
            message, f"Signal #{sid} is already {rec.status}."
        )
        return

    await app_ctx.db.update_hits_and_status(sid, status="closed")
    await message_answer_logged(message, f"Signal #{sid} closed. Tracking stopped.")
