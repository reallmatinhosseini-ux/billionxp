...
from app.formatter import add_free_cta, add_vip_header, format_signal_standard
...
async def send_vip_signal(bot: Bot, settings: Settings, sig: ParsedSignal) -> Message:
    body = format_signal_standard(sig)
    text = add_vip_header(body)
    return await bot.send_message(chat_id=settings.vip_channel_id, text=text)


async def send_free_signal(bot: Bot, settings: Settings, sig: ParsedSignal) -> Message:
    body = format_signal_standard(sig)
